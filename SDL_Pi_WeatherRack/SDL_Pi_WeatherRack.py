...
SDL_INTERRUPT_CLICKS = 1 # Number of Interupts per Rain Bucket and Anemometer Clicks
...
def fuzzyCompare(compareValue, value):
	VARYVALUE = 0.05
	_return = False
   	if ( (value > (compareValue * (1.0-VARYVALUE)))  and (value < (compareValue *(1.0+VARYVALUE))) ): _return = True
	return _return
...
def voltageToDegrees(value, defaultWindDirection):
	...
	ADJUST3OR5 = 1.0
	PowerVoltage = 5.0
	_return = defaultWindDirection
	if (fuzzyCompare(3.84 * ADJUST3OR5, value)): _return = 0.0
	if (fuzzyCompare(1.98 * ADJUST3OR5, value)): _return = 22.5
	if (fuzzyCompare(2.25 * ADJUST3OR5, value)): _return = 45.0
	if (fuzzyCompare(0.41 * ADJUST3OR5, value)): _return = 67.5
	if (fuzzyCompare(0.45 * ADJUST3OR5, value)): _return = 90.0
	if (fuzzyCompare(0.32 * ADJUST3OR5, value)): _return = 112.5
	if (fuzzyCompare(0.90 * ADJUST3OR5, value)): _return = 135.0
	if (fuzzyCompare(0.62 * ADJUST3OR5, value)): _return = 157.5
	if (fuzzyCompare(1.40 * ADJUST3OR5, value)): _return = 180.0
	if (fuzzyCompare(1.19 * ADJUST3OR5, value)): _return = 202.5
	if (fuzzyCompare(3.08 * ADJUST3OR5, value)): _return = 225.0
	if (fuzzyCompare(2.93 * ADJUST3OR5, value)): _return = 247.5
	if (fuzzyCompare(4.62 * ADJUST3OR5, value)): _return = 270.0
	if (fuzzyCompare(4.04 * ADJUST3OR5, value)): _return = 292.5
	if (fuzzyCompare(4.34 * ADJUST3OR5, value)): _return = 315.0 # chart in manufacturers documentation wront
	if (fuzzyCompare(3.43 * ADJUST3OR5, value)):  _return = 337.5
	return _return  # return previous value if not found

# return current microseconds
def micros():
	microseconds = int(round(time() * 1000000))
	return microseconds
...
class SDL_Pi_WeatherRack:
	...
	def setWindMode(self, selectedMode, sampleTime): # time in seconds 
  		...
  		SDL_Pi_WeatherRack._selectedMode = selectedMode
   		if (SDL_Pi_WeatherRack._selectedMode == SDL_MODE_SAMPLE): self.startWindSample(SDL_Pi_WeatherRack._sampleTime)
	...
	def current_wind_speed(self): # in milliseconds
		if (SDL_Pi_WeatherRack._selectedMode == SDL_MODE_SAMPLE):
			...
			# km/h * 1000 msec
			SDL_Pi_WeatherRack._currentWindCount = 0
			sleep(SDL_Pi_WeatherRack._sampleTime*1000)
	...
	def get_wind_gust(self):
		...
		_return = 0
		if (time != 0): _return = (1.0/float(time))*WIND_FACTOR
		return _return
	...
	def serviceInterruptRain(self,channel):
		...
		if(currentTime>500):   # debounce
			...
			if(currentTime<SDL_Pi_WeatherRack._currentRainMin):	SDL_Pi_WeatherRack._currentRainMin=currentTime
